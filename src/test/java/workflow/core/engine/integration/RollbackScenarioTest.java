 ...
 * Validates all rollback levels: node, step, and workflow
 *
 * Financial-Grade Requirements:
 * - Rollback must restore consistent state
 * - Compensation handlers must be invoked
...
public class RollbackScenarioTest {
    ...
    public void testNodeLevelRollback() {
        ...
        RollbackCoordinator.RollbackResult result = rollbackCoordinator.rollbackNode(
                executionId,
                "payment-node",
    ...
    public void testStepLevelRollback() {
        ...
        // Register handlers
        compensationService.registerHandler("task", context -> {
        ...
        RollbackCoordinator.RollbackResult result = rollbackCoordinator.rollbackToCheckpoint(
                executionId,
                checkpoint,
                RollbackCoordinator.RollbackReason.executionFailed("Downstream validation failed")
        );

        // Then: Should rollback nodes after checkpoint
        assertThat(result.isSuccess()).isTrue();
    ...
    public void testWorkflowLevelRollback() {
        ...
        RollbackCoordinator.RollbackResult result = rollbackCoordinator.rollbackWorkflow(
                executionId,
    ...
    private void setupWorkflowWithCompletedNode(String executionId) {
        WorkflowInstanceEntity workflow = new WorkflowInstanceEntity(
                executionId, "test-workflow", "1.0", "default");
        workflow.start();
