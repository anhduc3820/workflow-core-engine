...
/**
 * Repository: Workflow Instance (v2) Provides persistence operations for workflow instances
 * Supports optimistic locking for concurrent updates Multi-tenant aware
 */
@Repository
public interface WorkflowInstanceRepository extends JpaRepository<WorkflowInstanceEntity, String> {

  /**
   * Find workflow instance with pessimistic write lock Used for acquiring exclusive access during
   * execution
   */
  ...
  List<WorkflowInstanceEntity> findByWorkflowIdAndTenantIdOrderByCreatedAtDesc(
      String workflowId, String tenantId);
  ...
  @Query(
      "SELECT w FROM WorkflowInstanceEntity w WHERE w.state = 'RUNNING' "
          + "AND w.lockOwner IS NOT NULL "
          + "AND w.lockAcquiredAt < :staleThreshold")
  List<WorkflowInstanceEntity> findStaleRunningInstances(
  ...
  @Query(
      "SELECT w FROM WorkflowInstanceEntity w WHERE w.state = 'RUNNING' "
          ...
          + "AND w.lockOwner IS NOT NULL "
          + "AND w.lockAcquiredAt < :staleThreshold")
  List<WorkflowInstanceEntity> findStaleRunningInstancesByTenant(
      @Param("tenantId") String tenantId, @Param("staleThreshold") Instant staleThreshold);
