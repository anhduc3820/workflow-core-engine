 ...
 * Provides access to execution events for replay and audit
 */
@Repository
public interface ExecutionEventRepository extends JpaRepository<ExecutionEventEntity, Long> {
     ...
     */
    @Query("SELECT e FROM ExecutionEventEntity e WHERE e.executionId = :executionId " +
    ...
    Optional<ExecutionEventEntity> findLastEventByExecutionId(@Param("executionId") String executionId);
     ...
     */
    List<ExecutionEventEntity> findByExecutionIdAndStatus(String executionId, ExecutionEventStatus status);
