...
/** Repository: Node Execution Provides persistence operations for node execution records */
@Repository
public interface NodeExecutionRepository extends JpaRepository<NodeExecutionEntity, Long> {
  ...
  @Query(
      "SELECT n FROM NodeExecutionEntity n WHERE n.workflowInstance.executionId = :executionId ORDER BY n.executedAt ASC")
  ...
  @Query(
      "SELECT n FROM NodeExecutionEntity n WHERE n.workflowInstance.executionId = :executionId "
  ...
  @Query(
      "SELECT CASE WHEN COUNT(n) > 0 THEN TRUE ELSE FALSE END FROM NodeExecutionEntity n "
  ...
  @Query(
      "SELECT COUNT(n) FROM NodeExecutionEntity n WHERE n.workflowInstance.executionId = :executionId "
