...
/** Execution Replay REST API Provides endpoints for visual execution replay and analysis */
@Slf4j
@RestController
...
public class ExecutionReplayController {
  ...
  /** Get full execution replay GET /api/v1/replay/{executionId} */
  ...
  public ResponseEntity<VisualExecutionReplayService.ExecutionReplayData> getExecutionReplay(
    ...
    VisualExecutionReplayService.ExecutionReplayData replay =
        replayService.getExecutionReplay(executionId);
    return ResponseEntity.ok(replay);
  }

  /**
   * Get execution replay for specific sequence range GET
   * /api/v1/replay/{executionId}/range?start={start}&end={end}
   */
  ...
  public ResponseEntity<VisualExecutionReplayService.ExecutionReplayData> getExecutionReplayRange(
      @PathVariable String executionId, @RequestParam Long start, @RequestParam Long end) {
  ...
  /** Get node execution states GET /api/v1/replay/{executionId}/nodes */
  ...
  /** Get edge traversals GET /api/v1/replay/{executionId}/edges */
