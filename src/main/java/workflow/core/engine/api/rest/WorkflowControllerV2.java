...
/**
 * REST API: Workflow Operations (V2 - HA Ready) Clean Architecture - Uses application layer use
 * cases
 */
@Slf4j
@RestController
...
public class WorkflowControllerV2 {
  ...
  /** Deploy workflow definition POST /api/v2/workflows/deploy */
  @PostMapping("/deploy")
  ...
  /** Get all active workflows GET /api/v2/workflows */
  @GetMapping
  public ResponseEntity<List<WorkflowSummaryDTO>> getAllWorkflows() {
    ...
    List<WorkflowSummaryDTO> response =
        workflows.stream().map(this::toSummaryDTO).collect(Collectors.toList());

    return ResponseEntity.ok(response);
  }

  /** Get workflow definition by ID GET /api/v2/workflows/{workflowId} */
  ...
  /** Execute workflow (async) POST /api/v2/workflows/{workflowId}/execute */
  ...
  /** Get execution status GET /api/v2/workflows/executions/{executionId} */
  ...
  public ResponseEntity<ExecutionStatusDTO> getExecutionStatus(@PathVariable String executionId) {
    ...
    try {
      ...
      response.setExecutionHistory(
          history.stream().map(this::toNodeExecutionDTO).collect(Collectors.toList()));
  ...
  /** Resume paused execution POST /api/v2/workflows/executions/{executionId}/resume */
  ...
  public ResponseEntity<ExecutionResponseDTO> resumeExecution(@PathVariable String executionId) {
    ...
    try {
      ...
      WorkflowDefinitionEntity definition =
          deployWorkflowUseCase
              .getById(instance.getWorkflowId())
  ...
  /** Undeploy workflow DELETE /api/v2/workflows/{workflowId} */
