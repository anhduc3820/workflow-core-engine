...
public class GatewayHandler implements NodeHandler {
        ...
        switch (gatewayType) {
            ...
            default:
                throw new NodeExecutionException(node.getId(),
                        "Unsupported gateway type: " + gatewayType);
        }

        context.recordNodeExecution(node.getId(), NodeState.COMPLETED,
                "Gateway processed: " + gatewayType);
    }

    /**
     * Handle XOR (Exclusive) Gateway
     * Takes exactly one path based on conditions
     */
    ...
    /**
     * Handle AND (Parallel) Gateway
     * Takes all paths in parallel
     */
    ...
    /**
     * Handle OR (Inclusive) Gateway
     * Takes one or more paths based on conditions
     */
