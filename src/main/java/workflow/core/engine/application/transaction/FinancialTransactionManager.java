 ...
 *
 * Enforces ACID guarantees for workflow execution:
 * - Atomicity: All-or-nothing node execution
 * - Consistency: State transitions maintain invariants
 * - Isolation: SERIALIZABLE by default for financial operations
 * - Durability: All commits are persisted before acknowledgment
 *
 * Key Features:
...
public class FinancialTransactionManager {
    ...
    public <T> T executeInTransaction(TransactionContext context, TransactionOperation<T> operation) {
        ...
        recordTransactionEvent(context.getExecutionId(), transactionId,
                ExecutionEventType.TRANSACTION_STARTED, context.getNodeId());

        // Create transaction definition
        DefaultTransactionDefinition def = new DefaultTransactionDefinition();
        def.setName(transactionId);
        def.setIsolationLevel(context.getIsolationLevel() != null ?
                context.getIsolationLevel() : FINANCIAL_ISOLATION_LEVEL);
        ...
        ActiveTransaction activeTransaction = new ActiveTransaction(
                transactionId, context.getExecutionId(), context.getNodeId(), System.currentTimeMillis());
        ...
        try {
            ...
            if (result == null && context.isNullResultForbidden()) {
                throw new TransactionValidationException("Operation returned null result (forbidden in financial context)");
            }
            ...
            recordTransactionEvent(context.getExecutionId(), transactionId,
                    ExecutionEventType.TRANSACTION_COMMITTED, context.getNodeId());
            ...
            try {
                ...
                recordTransactionEvent(context.getExecutionId(), transactionId,
                        ExecutionEventType.TRANSACTION_ROLLED_BACK, context.getNodeId());

            } catch (Exception rollbackException) {
                log.error("CRITICAL: Rollback failed for transaction: {}", transactionId, rollbackException);
                throw new TransactionRollbackException("Failed to rollback transaction: " + transactionId, rollbackException);
            }
    ...
    /**
     * Execute with two-phase commit (for compensatable operations)
     * Phase 1: Prepare and validate
     * Phase 2: Commit or compensate
     */
    public <T> T executeWithTwoPhaseCommit(TransactionContext context,
                                            TwoPhaseOperation<T> operation) {
        String transactionId = generateTransactionId(context);
        ...
        T preparedResult;
        try {
            preparedResult = executeInTransaction(context, ctx -> {
                ...
                if (operation.hasCompensation()) {
                    compensationService.registerNodeCompensation(
                            ctx.getExecutionId(),
                            ctx.getNodeId(),
                            operation.getCompensationHandler()
                    );
                }
                return result;
            });
        ...
        try {
            ...
            log.error("Phase 2 (COMMIT) failed for transaction: {} - Initiating compensation", transactionId, e);
            ...
            try {
                ...
                log.error("CRITICAL: Compensation failed for transaction: {}", transactionId, compensationException);
                throw new CompensationFailureException(
                        "Two-phase commit COMMIT phase failed and compensation also failed", compensationException);
            }

            throw new TransactionFailureException("Two-phase commit failed in COMMIT phase (compensated)", e);
        }
    }

    /**
     * Check if idempotency key already exists
     * Returns true if operation should be skipped (already executed)
     */
    ...
    private void recordTransactionEvent(String executionId, String transactionId,
                                        ExecutionEventType eventType, String nodeId) {
        try {
    ...
    public static class ActiveTransaction {
        ...
        public ActiveTransaction(String transactionId, String executionId, String nodeId, long startTimeMs) {
