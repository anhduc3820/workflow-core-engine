...
/**
 * Rollback Coordinator Orchestrates multi-level rollback strategies for financial-grade workflow
 * execution
 *
 * <p>Rollback Levels: 1. Node-level: Rollback single node execution 2. Step-level: Rollback
 * sequence of nodes up to a checkpoint 3. Workflow-level: Rollback entire workflow instance
 *
 * <p>Each rollback: - Restores persisted state - Restores workflow variables - Executes
 * compensation handlers - Creates immutable audit trail
 */
@Slf4j
@Service
...
public class RollbackCoordinator {
  ...
  /**
   * Execute node-level rollback Compensates single node and restores state to before node execution
   */
  @Transactional
  public RollbackResult rollbackNode(String executionId, String nodeId, RollbackReason reason) {
    log.info(
        "Initiating node-level rollback: execution={}, node={}, reason={}",
        executionId,
        nodeId,
        reason);
    ...
    try {
      ...
      Optional<ExecutionEventEntity> lastCompletedEvent =
          nodeEvents.stream()
      ...
      if (!compensationResult.success()) {
        ...
        return RollbackResult.failure("Compensation failed: " + compensationResult.message());
      }
  ...
  /**
   * Execute step-level rollback Compensates sequence of nodes from current position back to
   * checkpoint
   */
  @Transactional
  public RollbackResult rollbackToCheckpoint(
      String executionId, Long checkpointSequence, RollbackReason reason) {
    log.info(
        "Initiating step-level rollback: execution={}, checkpoint={}, reason={}",
        executionId,
        checkpointSequence,
        reason);
    ...
    try {
      ...
      List<ExecutionEventEntity> eventsToRollback =
          eventService.getExecutionTimeline(executionId).stream()
      ...
      log.info(
          "Rolling back {} nodes to checkpoint {}", eventsToRollback.size(), checkpointSequence);
  ...
  /** Execute workflow-level rollback Compensates entire workflow and resets to initial state */
  @Transactional
  public RollbackResult rollbackWorkflow(String executionId, RollbackReason reason) {
    ...
    try {
      ...
      String preRollbackState =
          instance.getState() != null ? instance.getState().name() : "UNKNOWN";
      String preRollbackVariables = instance.getCurrentVariables();

      // Get all completed node events
      List<ExecutionEventEntity> completedEvents =
          eventService.getExecutionTimeline(executionId).stream()
              .filter(e -> e.getEventType() == ExecutionEventType.NODE_COMPLETED)
              .sorted(Comparator.comparing(ExecutionEventEntity::getSequenceNumber).reversed())
              .collect(Collectors.toList());
  ...
  public Long createCheckpoint(String executionId, String checkpointName) {
    ...
    ExecutionEventEntity checkpointEvent =
  ...
  public List<CheckpointInfo> getCheckpoints(String executionId) {
    List<ExecutionEventEntity> checkpointEvents =
        eventService.getExecutionTimeline(executionId).stream()
    ...
    return checkpointEvents.stream()
        .map(
            e ->
                new CheckpointInfo(
                    e.getSequenceNumber(), e.getNodeName(), e.getTimestamp().toString()))
        .collect(Collectors.toList());
  }
  ...
  private void recordRollbackEvent(
      String executionId, String nodeId, ExecutionEventType eventType, RollbackReason reason) {
    Map<String, Object> eventData = new HashMap<>();
