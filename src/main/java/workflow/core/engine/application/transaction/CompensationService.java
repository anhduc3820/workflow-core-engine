 ...
 * Handles compensating actions for rollback scenarios
 *
 * Compensation Strategies:
 * 1. Node-level compensation (undo single node)
 * 2. Step-level compensation (undo sequence of nodes)
 * 3. Workflow-level compensation (undo entire workflow)
 *
 * Each compensable action must register a compensation handler
 */
@Slf4j
@Service
...
public class CompensationService {
    ...
    private final Map<String, CompensationHandler> nodeCompensationHandlers = new ConcurrentHashMap<>();
     ...
     */
    public void registerNodeCompensation(String executionId, String nodeId, CompensationHandler handler) {
    ...
    public CompensationResult compensateNode(String executionId, String nodeId) {
        ...
        Optional<ExecutionEventEntity> lastEvent = nodeEvents.stream()
        ...
        CompensationHandler handler = nodeCompensationHandlers.getOrDefault(nodeKey,
                compensationHandlers.get(nodeType));
        ...
        ExecutionEventEntity compensationEvent = eventService.recordEvent(
                executionId, ExecutionEventType.COMPENSATION_INITIATED, eventData);
        ...
        try {
            ...
            CompensationContext context = new CompensationContext(
                    ...
                    nodeId,
                    nodeType,
                    completedEvent.getOutputSnapshot()
            );
     ...
     */
    @Transactional
    public List<CompensationResult> compensateSequence(String executionId, String startNodeId, String endNodeId) {
        log.info("Compensating sequence from {} to {} in execution: {}", startNodeId, endNodeId, executionId);
     ...
     */
    private List<String> extractNodesInSequence(List<ExecutionEventEntity> events,
    ...
    public static class CompensationContext {
        ...
        public CompensationContext(String executionId, String nodeId, String nodeType,
                                   String originalOutputSnapshot) {
        ...
        public String getOriginalOutputSnapshot() { return originalOutputSnapshot; }
    }
