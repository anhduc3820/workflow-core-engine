...
/**
 * Replay Engine Reconstructs workflow state from persisted execution events
 *
 * <p>Key Guarantees: - Deterministic replay (same events = same result) - Crash recovery (replay
 * after pod restart) - State reconstruction from events only - No dependency on in-memory state
 */
@Slf4j
@Service
...
public class ReplayEngine {
  ...
  /**
   * Reconstruct workflow state from events Returns the state as it was at a specific sequence
   * number
   */
  @Transactional(readOnly = true)
  public ReconstructedState reconstructState(String executionId, Long upToSequence) {
    log.info(
        "Reconstructing state for execution: {} up to sequence: {}", executionId, upToSequence);
  ...
  private ReconstructedState reconstructStateFromEvents(
      String executionId, List<ExecutionEventEntity> events) {
  ...
  public boolean canResume(String executionId) {
    ...
    return state.getWorkflowState().equals("RUNNING") && state.getCurrentNodeId() != null;
  }
  ...
  /** Validate replay consistency Replays twice and ensures identical results */
  @Transactional(readOnly = true)
  ...
  public static class ReconstructedState {
    ...
    public void markNodeStarted(String nodeId, String timestamp) {
      nodeStates.computeIfAbsent(nodeId, k -> new NodeState(nodeId)).setStartTime(timestamp);
    }

    public void markNodeCompleted(String nodeId, String timestamp) {
      nodeStates
          .computeIfAbsent(nodeId, k -> new NodeState(nodeId))
    ...
    public void setLastSequenceNumber(Long seq) {
    ...
    public Map<Long, String> getCheckpoints() {
      return checkpoints;
    }
