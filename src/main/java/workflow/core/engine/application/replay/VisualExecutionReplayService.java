...
/**
 * Visual Execution Replay Service
 * Provides deterministic replay of workflow execution for visual frontend
 *
 * Key Features:
 * - Step-by-step replay
 * - Timeline-based replay
 * - Node highlighting based on execution state
...
public class VisualExecutionReplayService {
    ...
    public ExecutionReplayData getExecutionReplay(String executionId, Long startSeq, Long endSeq) {
        List<ExecutionEventEntity> events = eventService.getExecutionTimeline(executionId, startSeq, endSeq);

        List<ReplayStep> steps = events.stream()
                .map(event -> new ReplayStep(
                        event.getSequenceNumber(),
                        event.getSequenceNumber(),
                        event.getEventType().name(),
                        event.getNodeId(),
    ...
    /**
     * Get node execution states for visualization
     * Returns map of nodeId -> execution state
     */
    @Transactional(readOnly = true)
    public Map<String, NodeExecutionState> getNodeStates(String executionId) {
        ...
        for (ExecutionEventEntity event : events) {
            ...
            NodeExecutionState state = nodeStates.computeIfAbsent(
                    nodeId,
    ...
    public List<EdgeTraversal> getEdgeTraversals(String executionId) {
        ...
        for (ExecutionEventEntity event : events) {
            if (event.getEdgeTaken() != null) {
                traversals.add(new EdgeTraversal(
                        order++,
                        event.getEdgeTaken(),
                        event.getTimestamp().toString(),
                        event.getDecisionResult()
