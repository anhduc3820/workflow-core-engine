...
/**
 * Financial-Grade Execution Event Service Manages immutable execution events for replay and audit
 *
 * <p>Key Guarantees: - Idempotent event recording - Atomic event persistence - Deterministic replay
...
public class ExecutionEventService {
  ...
  /**
   * Record an execution event (idempotent) Returns the event if recorded, or existing event if
   * already exists
   */
  @Transactional
  public ExecutionEventEntity recordEvent(
      String executionId, ExecutionEventType eventType, Map<String, Object> eventData) {
  ...
  public void markEventCompleted(
      Long eventId, Long durationMs, Map<String, Object> outputSnapshot) {
    ExecutionEventEntity event =
        eventRepository
            .findById(eventId)
  ...
  public void markEventFailed(
      Long eventId, String errorMessage, Map<String, Object> errorSnapshot) {
    ExecutionEventEntity event =
        eventRepository
            .findById(eventId)
            .orElseThrow(() -> new IllegalArgumentException("Event not found: " + eventId));
    ...
    event.markFailed(
        serializeSnapshot(
            Map.of(
                ...
                errorMessage,
                "snapshot",
                errorSnapshot != null ? errorSnapshot : Map.of())));
    eventRepository.save(event);
  }
  ...
  @Transactional(readOnly = true)
  public List<ExecutionEventEntity> getExecutionTimeline(
      String executionId, Long startSeq, Long endSeq) {
  ...
  public boolean canResumeExecution(String executionId) {
    ...
    return lastEvent.isPresent() && lastEvent.get().getStatus() != ExecutionEventStatus.FAILED;
  }
  ...
  public ExecutionStatistics getStatistics(String executionId) {
    ...
    long completedEvents =
        events.stream().filter(e -> e.getStatus() == ExecutionEventStatus.COMPLETED).count();
    long failedEvents =
        events.stream().filter(e -> e.getStatus() == ExecutionEventStatus.FAILED).count();
    long totalDuration =
        events.stream()
  ...
  private Long getNextSequenceNumber(String executionId) {
    Optional<ExecutionEventEntity> lastEvent =
        eventRepository.findLastEventByExecutionId(executionId);
  ...
  public record ExecutionStatistics(
      long totalEvents, long completedEvents, long failedEvents, long totalDurationMs) {}
}
