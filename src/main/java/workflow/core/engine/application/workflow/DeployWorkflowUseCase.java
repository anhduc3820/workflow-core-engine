...
/** Use Case: Deploy Workflow Handles workflow definition deployment */
@Slf4j
@Service
...
public class DeployWorkflowUseCase {
  ...
  public WorkflowDefinitionEntity execute(String workflowJson) {
    ...
    Optional<WorkflowDefinitionEntity> existing =
        definitionRepository.findByWorkflowIdAndVersion(
            definition.getWorkflowId(), definition.getVersion());

    if (existing.isPresent()) {
      log.warn(
          "Workflow {} version {} already exists, updating",
    ...
    log.info(
        "Workflow {} version {} deployed successfully",
        definition.getWorkflowId(),
        definition.getVersion());
  ...
  public Optional<WorkflowDefinitionEntity> getById(String workflowId) {
    ...
    List<WorkflowDefinitionEntity> versions =
        definitionRepository.findByWorkflowIdAndTenantIdOrderByDeployedAtDesc(
            workflowId, "default");
  ...
  public void undeploy(String workflowId) {
    ...
    List<WorkflowDefinitionEntity> entities =
        definitionRepository.findByWorkflowIdAndTenantIdOrderByDeployedAtDesc(
            workflowId, "default");
