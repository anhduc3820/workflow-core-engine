...
/**
 * Use Case: Execute Workflow (Stateless, HA-Ready)
 * Core workflow execution orchestrator
 * Fully stateless - all state externalized to ExecutionStateManager
 * Supports horizontal scaling and HA
 */
@Slf4j
@Service
...
public class StatelessWorkflowExecutor {
    ...
    public String executeAsync(WorkflowGraph workflowGraph, Map<String, Object> initialVariables) {
        ...
        WorkflowInstanceEntity instance = stateManager.createInstance(
                workflowGraph.getWorkflowId(),
                workflowGraph.getVersion(),
                initialVariables
        );
     ...
     */
    public WorkflowInstanceEntity executeSync(WorkflowGraph workflowGraph, Map<String, Object> initialVariables) {
        // Create workflow instance
        WorkflowInstanceEntity instance = stateManager.createInstance(
                workflowGraph.getWorkflowId(),
                workflowGraph.getVersion(),
        ...
        try {
            ...
            if (instance.getCurrentNodeId() != null) {
                ...
                if (currentNode != null) {
                    ...
                    stateManager.failWorkflow(executionId, "Current node not found in workflow",
    ...
    @Async
    @Transactional
    protected CompletableFuture<Void> executeWorkflowAsync(String executionId, WorkflowGraph workflowGraph) {
     ...
     */
    private void continueToNextNodes(String executionId, GraphNode currentNode, WorkflowGraph workflowGraph) {
        ...
        List<GraphEdge> edgesToTake = nodeExecutorService.selectEdges(currentNode, outgoingEdges, variables);
