...
/**
 * Service: Node Executor Handles individual node execution logic Separated from main executor for
 * better testability
 */
@Slf4j
@Service
...
public class NodeExecutorService {
  ...
  public void execute(
      GraphNode node, NodeExecutionEntity execution, Map<String, Object> variables) {
    NodeHandler handler = findHandler(node);

    if (handler == null) {
      throw new NodeExecutionException(
          node.getId(), "No handler found for node type: " + node.getType());
    }
  ...
  public List<GraphEdge> selectEdges(
      GraphNode node, List<GraphEdge> outgoingEdges, Map<String, Object> variables) {
    ...
    if (node.getType() == NodeType.EXCLUSIVE_GATEWAY) {
      ...
      log.warn(
          "Node {} has multiple outgoing edges but is not a gateway, taking first edge",
          node.getId());
  ...
  private List<GraphEdge> selectExclusiveGatewayEdge(
      List<GraphEdge> edges, Map<String, Object> variables) {
    ...
    throw new IllegalStateException(
        "No edge condition satisfied in XOR gateway and no default edge found");
  }

  /** Select edges for AND gateway (all edges) */
  private List<GraphEdge> selectParallelGatewayEdges(
      List<GraphEdge> edges, Map<String, Object> variables) {
  ...
  private List<GraphEdge> selectInclusiveGatewayEdges(
      List<GraphEdge> edges, Map<String, Object> variables) {
    ...
    if (selectedEdges.isEmpty()) {
      ...
      throw new IllegalStateException(
          "No edge condition satisfied in OR gateway and no default edge found");
    }
  ...
  private NodeHandler findHandler(GraphNode node) {
    return nodeHandlers.stream().filter(handler -> handler.supports(node)).findFirst().orElse(null);
  }
}
