...
/** Workflow Metrics Service (v2) Exposes Prometheus-compatible metrics for observability */
@Slf4j
@Service
public class WorkflowMetricsService {

  private final MeterRegistry meterRegistry;
  private final ConcurrentHashMap<String, AtomicInteger> activeWorkflowsByTenant =
  ...
  public void recordWorkflowStarted(String tenantId, String workflowId) {
    ...
    activeWorkflowsByTenant
        .computeIfAbsent(
            tenantId,
            k ->
                meterRegistry.gauge(
                    "workflow.active.count", new AtomicInteger(0), AtomicInteger::get))
        .incrementAndGet();
  }
  ...
  public void recordRuleExecution(
      String tenantId, String ruleFlowGroup, int rulesFired, long durationMs) {
