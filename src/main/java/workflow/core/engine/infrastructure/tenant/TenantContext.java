...
/**
 * Tenant Context Holder (v2) Thread-local storage for current tenant ID Used for multi-tenant
 * isolation throughout the application
 */
@Slf4j
public class TenantContext {
  ...
  public static <T> T executeWithTenant(String tenantId, java.util.concurrent.Callable<T> action)
      throws Exception {
    String previousTenant = getTenantId();
    try {
      setTenantId(tenantId);
