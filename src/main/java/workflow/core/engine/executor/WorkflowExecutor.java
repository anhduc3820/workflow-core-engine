...
/**
 * Core workflow execution engine Orchestrates workflow execution in a deterministic, node-driven
 * manner
 */
@Slf4j
@Service
...
public class WorkflowExecutor {
  ...
  private void continueExecution(
      GraphNode currentNode, WorkflowGraph graph, WorkflowContext context) {
  ...
  private List<GraphEdge> selectEdgesToTake(
      GraphNode node, List<GraphEdge> edges, WorkflowContext context) {
  ...
  private List<GraphEdge> selectExclusiveGatewayEdge(
      List<GraphEdge> edges, WorkflowContext context) {
    // Sort by priority
    ...
    for (GraphEdge edge : sortedEdges) {
      if (evaluateEdgeCondition(edge, context)) {
        log.debug(
            "XOR Gateway: Selected edge {} (condition: {})", edge.getId(), edge.getCondition());
  ...
  private boolean evaluateEdgeCondition(GraphEdge edge, WorkflowContext context) {
    ...
    try {
      ...
      log.warn(
          "Simple evaluation failed for condition: {}, trying full evaluation",
          edge.getCondition());
