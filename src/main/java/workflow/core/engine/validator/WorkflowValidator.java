 ...
 * Ensures workflow integrity and executability
 */
@Slf4j
@Service
public class WorkflowValidator {
    ...
    private void validateStartEvent(WorkflowGraph graph, ValidationResult result) {
        ...
        if (outgoing.isEmpty()) {
            result.addError("START_EVENT_NO_OUTGOING",
                    "START_EVENT must have at least one outgoing edge");
        }

        List<GraphEdge> incoming = graph.getIncomingEdges(graph.getStartEvent().getId());
        if (!incoming.isEmpty()) {
            result.addError("START_EVENT_HAS_INCOMING",
                    "START_EVENT must not have incoming edges");
        }
    }
        ...
        for (GraphNode node : graph.getAllNodes()) {
            ...
            for (GraphEdge edge : graph.getOutgoingEdges(nodeId)) {
                if (edge.getSource().equals(edge.getTarget())) {
                    result.addError("SELF_LOOP",
                            "Node '" + node.getName() + "' has a self-loop edge");
                }

                // Verify target exists
                if (graph.getNode(edge.getTarget()) == null) {
                    result.addError("EDGE_TARGET_NOT_FOUND",
    ...
    private void validateGateways(WorkflowGraph graph, ValidationResult result) {
        for (GraphNode node : graph.getAllNodes()) {
            ...
            if (gatewayType == null) {
                result.addError("GATEWAY_TYPE_MISSING",
                        "Gateway '" + node.getName() + "' must specify gatewayType");
                continue;
            }
            ...
            // Mixed gateway
                ...
                result.addWarning("GATEWAY_MIXED",
                        "Gateway '" + node.getName() + "' is both diverging and converging");
            }
            else {
                result.addWarning("GATEWAY_INVALID_STRUCTURE",
                        "Gateway '" + node.getName() + "' has invalid structure");
            }
        }
    }
     ...
     */
    private void validateDivergingGateway(GraphNode gateway, List<GraphEdge> outgoing,
                                          GatewayType gatewayType, ValidationResult result) {
        if (gatewayType == GatewayType.XOR || gatewayType == GatewayType.OR) {
            ...
            for (GraphEdge edge : outgoing) {
                if (!edge.hasCondition()) {
                    if (!hasDefaultPath) {
                        ...
                        result.addWarning("GATEWAY_MULTIPLE_DEFAULT",
            ...
            if (!hasDefaultPath) {
                result.addWarning("GATEWAY_NO_DEFAULT",
    ...
    private void validateReachability(WorkflowGraph graph, ValidationResult result) {
        ...
        for (GraphNode node : graph.getAllNodes()) {
            if (!reachableFromStart.contains(node.getId())) {
                result.addWarning("NODE_UNREACHABLE",
                        "Node '" + node.getName() + "' is unreachable from START_EVENT");
            }
        }
        ...
        if (!endEventReachable && !graph.getEndEvents().isEmpty()) {
            result.addError("NO_REACHABLE_END_EVENT",
                    "No END_EVENT is reachable from START_EVENT");
        }
    }
    ...
    private void validateBusinessRuleTasks(WorkflowGraph graph, ValidationResult result) {
        for (GraphNode node : graph.getAllNodes()) {
            ...
            if (config.getRuleFile() == null || config.getRuleFile().trim().isEmpty()) {
                result.addError("RULE_TASK_NO_FILE",
            ...
            if (config.getRuleflowGroup() == null || config.getRuleflowGroup().trim().isEmpty()) {
                result.addError("RULE_TASK_NO_GROUP",
    ...
    private void validateServiceTasks(WorkflowGraph graph, ValidationResult result) {
        for (GraphNode node : graph.getAllNodes()) {
            ...
            if (config.getServiceName() == null || config.getServiceName().trim().isEmpty()) {
                result.addError("SERVICE_TASK_NO_NAME",
